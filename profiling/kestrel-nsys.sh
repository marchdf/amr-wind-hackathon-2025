...
cmd "which amr_wind"
cmd "which nsys"
PROFNAME="profiling-nsys"
cmd "rm -rf ${PROFNAME}-*.nsys-rep ${PROFNAME}-*.qdstrm  ${PROFNAME}-*.sqlite"
cmd "srun -N ${SLURM_JOB_NUM_NODES} -n ${RANKS} --ntasks-per-node=${NTASKS_PER_NODE} --gpus-per-node=4 --gpu-bind=closest nsys profile --stats=true -o ${PROFNAME}-${SLURM_PROCID} amr_wind demo_case.inp amrex.abort_on_out_of_gpu_memory=1 amrex.the_arena_is_managed=0 amr.blocking_factor=16 amr.max_grid_size=128 amrex.use_profiler_syncs=0 amrex.async_out=0 amrex.use_gpu_aware_mpi=1 time.max_step=40040 > out-nsys.log"
