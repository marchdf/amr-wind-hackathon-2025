...
cmd "srun -N ${SLURM_JOB_NUM_NODES} -n ${RANKS} --ntasks-per-node=${NTASKS_PER_NODE} --gpus-per-node=4 --gpu-bind=closest nsys profile --stats=true --trace=nvtx,cuda,mpi --mpi-impl=mpich -o report%q{SLURM_PROCID} amr_wind demo_case.inp amrex.abort_on_out_of_gpu_memory=1 amrex.the_arena_is_managed=0 amr.blocking_factor=16 amr.max_grid_size=128 amrex.use_profiler_syncs=0 amrex.async_out=0 amrex.use_gpu_aware_mpi=1 time.max_step=40005 mac_proj.verbose=1 mac_proj.bottom_verbose=1 nodal_proj.verbose=1 nodal_proj.bottom_verbose=1 > out-nsys-${SLURM_JOB_ID}.log"
